...
public class CarReasoner extends NormEngine {
	...
	/**
	 * Does reasoning to activate rules in base of the facts in the knowledge base. TODO: MASSIMO
	 */
	public CarAction decideAction(Car car, CarContext context, boolean [] lane) {
		boolean violate;
		boolean wrong=false;
		int violateProb;
		this.state = CarReasonerState.NoNormActivated;
		...
		/* LOOK IF THE CAR IS IN A VALID LANE */
		for (int i=0;i<3;i++){
			if (lane[i]==false && car.getX()-8==i){
				wrong=true;
			}
		}
		if (wrong==false){
